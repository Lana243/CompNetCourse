...
window = sg.Window(title="Получатель UDP", layout=layout, margins=(100, 50))
...
while True:
    ...
    try:
        ...
        total_packets = int(server_socket.recvfrom(1024)[0].decode('utf-8'))
        print(total_packets)
        for i in range(total_packets):
            try:
                packet_time_ms = server_socket.recvfrom(1024)[0].decode('utf-8').split()
                print(i, packet_time_ms)
                packets_recv += 1
                if start_time == 0:
                    start_time = int(packet_time_ms[0])
